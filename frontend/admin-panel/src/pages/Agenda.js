  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#1a5c2a' }}>Agenda Kegiatan</h1>
        <button onClick={() => { setShowForm(!showForm); setEditingId(null); setForm({ judul: '', tanggal: '', jam_mulai: '', jam_selesai: '', deskripsi: '', lokasi: '', is_published: 1 }); }}
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ background: 'white', padding: 24, borderRadius: 12, marginBottom: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div><label style={labelStyle}>Jam Mulai</label><input type="time" value={form.jam_mulai} onChange={e => setForm({...form, jam_mulai: e.target.value})} style={inputStyle} /></div>
            ...
            <div><label style={labelStyle}>Lokasi</label><input value={form.lokasi} onChange={e => setForm({...form, lokasi: e.target.value})} style={inputStyle} /></div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#f8f9fa' }}>
              ...
              <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: '0.85rem', fontWeight: 600 }}>Lokasi</th>
          ...
          <tbody>
            {agendaList.map(item => (
              <tr key={item.id} style={{ borderTop: '1px solid #f0f0f0' }}>
                ...
                <td style={{ padding: '12px 16px', fontSize: '0.9rem' }}>{item.lokasi || '-'}</td>
